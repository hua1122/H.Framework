...
namespace MessageQueue.Rabbit
{
    ...
    public class RabbitMqService : IDisposable
    {
        ...
        private const string _rabbitMqAttributeString = "RabbitMqAttribute";
        ...
        private static RabbitMqAttribute GetRabbitMqAttribute<T>()
        {
            if (_rabbitMqAttribute == null)
            {
                ...
                _rabbitMqAttribute = typeOfT.GetCustomAttributes(typeof(RabbitMqAttribute), false).FirstOrDefault() as RabbitMqAttribute;
            }
        ...
        public void Publish<T>(T command) where T : class
        {
            ...
            if (queueInfo == null)
                throw new ArgumentException(_rabbitMqAttributeString);
        ...
        {
            //var queueInfo = typeof(T).GetCustomAttributes(typeof(RabbitMqAttribute), false).FirstOrDefault() as RabbitMqAttribute;
            var queueInfo = GetRabbitMqAttribute<T>();
            if (queueInfo == null)
                throw new ArgumentException(_rabbitMqAttributeString);
        ...
        public void Subscribe<T>(string queue, bool isProperties, Action<T> handler, bool isDeadLetter) where T : class
        {
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                {
                    if (!isDeadLetter)
                        PublishToDead<DeadLetterQueue>(queue, msg, ex);
                }
                finally
                {
        ...
        private void Pull<T>(string exchange, string queue, string routingKey, Action<T> handler) where T : class
        {
            ...
            var msg = JsonConvert.DeserializeObject<T>(Encoding.UTF8.GetString(result.Body));
            try
            {
                handler(msg);
            }
